...
wss.on('connection', (socket) => {
  ...
  const session = query({
    ...
    options: {
      ...
      effort: EFFORT,
      /*
       * Raised from 24. A news briefing that actually reads the bodies of
       * several newsletters spends two tool calls per message — a search and a
       * fetch — and 24 ran out partway through, leaving the turn to die rather
       * than answer. 40 covers that kind of work without being an open cheque.
       */
      maxTurns: 40,
