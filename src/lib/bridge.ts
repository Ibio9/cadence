...
/**
 * The to-do list, which the bridge reads and writes through us.
 *
 * The second request/reply channel, for the same reason as the first: the list
 * is in this browser's localStorage, so the bridge cannot touch it directly
...
function dispatch(ws: WebSocket) {
  ws.addEventListener('message', (e: MessageEvent) => {
    ...
    if (msg.type === 'ready') {
      ...
      onBlade?.(msg.blade)
    } else if (msg.type === 'tasks' && msg.id) {
      // Synchronous on our side — it is a read or a write against a store that
      // is already in memory — but it still answers through the same reply
      // envelope, because the bridge is holding a tool call open on it.
      const id = msg.id
      const answer = (payload: TaskResult) => {
