...
/**
 * The to-do list, answered the same way and bridge-only for the same reason.
 *
 * On the direct path this is simply absent rather than stubbed. The list is
 * still on screen and still his to edit by hand; what he loses without a
 * bridge is JARVIS being able to read it, and a handler registered against a
 * transport with no inbound channel would only hide that.
 */
export function watchTasks(fn: (req: bridge.TaskRequest) => bridge.TaskResult): void {
  if (usingBridge) bridge.watchTasks(fn)
}
