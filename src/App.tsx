...
import { TTS_ENGINE, PUSH_TO_TALK, PTT_RELEASE_GRACE_MS } from './config'
...
export default function App() {
  ...
  /**
   * The push-to-talk key, and how long its release still counts.
   *
   * A ref rather than state on purpose. `mode()` is called from inside the
   * voice loop's own callbacks, several times per segment, and it has to read
   * the value as it is right now — a state variable would hand it whatever was
   * current when the loop was created and never update.
   */
  const ptt = useRef({ held: false, until: 0 })
   ...
   *
   * With push-to-talk on, the key decides and the phase does not. Nothing is
   * transcribed unless SPACE is down or the release grace is still running,
   * which is the whole point: a microphone that is open only while he is
   * holding it cannot pick up the room, the television, or the other half of
   * a conversation he is having with somebody else.
   *
   * It also makes the echo gate redundant. 'guard' exists to stop JARVIS
   * hearing himself; in this mode he is deaf while speaking anyway, because
  ...
  const mode = (): VoiceMode => {
    ...
    if (PUSH_TO_TALK) {
      if (ptt.current.held) return 'command'
      return performance.now() < ptt.current.until ? 'command' : 'deaf'
    }
  ...
  /**
   * Start a turn from something that is not the microphone.
   *
   * Three things need this now: the briefing that fires on power-up, the M
   * gesture, and anything he types. All of them can land while JARVIS is
   * mid-sentence, so each one barges in the way a spoken interruption does
   * rather than queueing behind an answer he has stopped caring about.
   *
   * The interrupt is conditional because `cancel()` puts a message on the wire
   * whether or not a turn is in flight, and telling the bridge to stop doing
   * nothing is a needless round trip on every keystroke-ended sentence.
   */
  ...
  const powerOn = async () => {
    ...
    try {
      // The briefing is fired from inside ignite(), as soon as the bridge is
      // up, so it runs behind the boot animation instead of after it.
      await ignite()
    } catch (err) {
  ...
  const ignite = async () => {
    ...
    /**
     * The to-do list, answered out of the store.
     *
     * Registered with the other watchers rather than lazily, because the model
     * is told to read the list before planning anything and the first turn of
     * the session is a briefing that does exactly that.
     */
    watchTasks((req) => {
      const s = store.getState()
      if (req.op === 'add') {
    ...
    /**
     * The bridge first, because the briefing depends on it.
     *
     * This used to sit behind a flat 9.2 second wait that paced a four-beat
     * boot animation, and the briefing was not even requested until all of it
     * had finished. That is the wait Ibrahim was sitting through: the machine
     * was idle for nine seconds and only then started asking Gmail anything.
     *
     * Now the connection is established first and the request goes out while
     * the reactor is still lighting, so the mail is being read behind the
    ...
    /**
     * The opening briefing, fired into the boot rather than after it.
     *
     * Deliberately not awaited. Everything below — the analyser, the speech
     * probe, the voice loop — is independent of it and should be set up while
     * the model is working, not afterwards.
     *
     * This moves the phase to 'thinking' immediately, several seconds before
     * the overlay finishes. That is safe only because Boot.tsx owns its own
     * lifetime now and no longer unmounts the moment the phase changes; if you
     * ever make the overlay phase-driven again, this line is what will cut the
     * animation off mid-way.
     */
    startTurn(OPENING_BRIEFING)
    ...
    /**
     * Only if nothing else has taken the phase.
     *
     * The briefing above is in flight by now and has almost certainly moved
     * the machine to 'thinking' or 'speaking'. Setting 'dormant'
     * unconditionally would stand him down in the middle of his own first
     * sentence, with the answer still streaming in behind it.
     */
    if (store.getState().phase === 'boot') store.getState().setPhase('dormant')
  }
  ...
  /**
   * Right hand, three fingers up, thumb tucked: read the mail again.
   *
   * Polled rather than pushed, for the same reason GestureGuide polls: the
   * tracker publishes a plain mutable array at camera rate on purpose, and
   * routing a gesture through React state would re-render the whole tree
   * thirty times a second to catch one pose. Five times a second is faster
   * than he can make the shape and drop it.
   *
   * `emGesture` is latched on its own side, so holding the M fires once rather
   * than once per tick. Nothing here needs to know that, which is the point.
   */
  useEffect(() => {
    if (phase === 'offline' || phase === 'boot') return
    const poll = window.setInterval(() => {
      // Right hand. Content: what has arrived.
      if (hands.emGesture()) startTurn(REREAD_MAIL)
      if (hands.fourGesture()) startTurn(NEWS_BRIEFING)
      // Left hand. The interface itself: say it again, or show me the record.
      if (hands.oneGesture()) startTurn(OPENING_BRIEFING)
      if (hands.historyGesture()) store.getState().toggleHistory()
    }, 200)
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      // Space. Hold-to-talk when PUSH_TO_TALK is on, otherwise the old
      // press-to-start-a-turn, which is still useful when filming so a missed
      // wake word doesn't cost a take.
      ...
      if (phase === 'boot') return // the boot sequence owns the phase

      if (PUSH_TO_TALK) {
        /**
         * Down opens the microphone and nothing else does.
         *
         * `until` is cleared here as well as set on release, because pressing
         * again inside the grace window must not leave a stale deadline that
         * expires mid-sentence and cuts the mode back to deaf while he is
         * still speaking.
         */
        ptt.current.held = true
        ptt.current.until = 0
        // Pressing while he is talking is the barge-in. Same as speaking over
        // him was, which is the behaviour this replaces.
        if (phase === 'thinking' || phase === 'tooling' || phase === 'speaking') {
          onSpeechStart()
        }
        listen(AWAIT_SPEECH_MS)
    ...
    /**
     * Up closes it, after a grace period.
     *
     * The grace is not politeness. Speech is segmented by the detector and
     * transcribed afterwards, and `transcribe` re-reads the mode when the
     * audio comes back — so dropping to deaf on keyup would discard the
     * sentence that had just been spoken into the microphone. See
     * PTT_RELEASE_GRACE_MS.
     *
     * The stand-down is conditional on still being in 'listening'. Going
     * dormant unconditionally would bump the turn counter, and if the
     * transcription had already landed and started a turn, that turn would be
     * marked stale and silenced half a second after he finished talking.
     */
    const onKeyUp = (e: KeyboardEvent) => {
      if (!PUSH_TO_TALK || e.code !== 'Space') return
      const p = ptt.current
      if (!p.held) return
      p.held = false
      p.until = performance.now() + PTT_RELEASE_GRACE_MS
      clearIdle()
      idleTimer.current = setTimeout(() => {
        if (store.getState().phase === 'listening') goDormant()
      }, PTT_RELEASE_GRACE_MS + 1200)
    }
