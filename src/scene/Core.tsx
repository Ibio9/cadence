...
const fragment = /* glsl */ `
  ...
  void main() {
    ...
    /*
     * Three hues travelling around the ring, rather than one flat body colour.
     *
     * This is what gives it the fluid, iridescent read rather than the single
     * cyan it had. Two offset sine bands sweep the circumference and drift on
     * uPhase, so no part of the ring holds the same colour for long and the
     * whole thing appears to move even when the geometry is still.
     *
     * The bands are angular, not radial, on purpose: a radial gradient reads
     * as a lit sphere, an angular one reads as something circulating.
     */
    float bandB = 0.5 + 0.5 * sin(a * 1.0 + uPhase * 0.38);
    float bandC = 0.5 + 0.5 * sin(a * 2.0 - uPhase * 0.27 + 2.1);
    vec3 body = mix(uColor, uColorB, bandB * 0.85);
    body = mix(body, uColorC, bandC * 0.55);

    // The moving highlights still run hot over the top of it.
    vec3 col = mix(body, uHot,
 ...
 */
/*
 * Down from 0.60, in stages, and this is the figure that changes what it is.
 *
 * At 0.60 the ring owned the middle of a wide screen and the transcript ran
 * across its brightest band, which no styling fixes: white words on a blown
...
export function Core({ drive }: { drive: Drive }) {
  ...
  const uniforms = useMemo(
    () => ({
      ...
      /*
       * The two companion hues. Violet and green either side of the interface
       * cyan: far enough apart to read as several colours, close enough in
       * luminance that no band of the ring goes dull against the others.
       *
       * Fixed rather than following the phase accent, which stays the
       * authority for the interface chrome. Tying these to it as well would
       * collapse the whole picture back to one hue every time the phase moved.
       */
      uColorB: { value: new THREE.Color('#7b5cff') },
      uColorC: { value: new THREE.Color('#25e0a6') },
      uLevel: { value: 0 },
      uPhase: { value: 0 },
      uOpen: { value: 0 },
