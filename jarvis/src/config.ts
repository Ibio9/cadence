...
/**
 * Hold a key to talk, instead of the wake word listening all the time.
 *
 * The wake word is the better demo and the worse tool. An always-open
 * microphone in a room with other people in it hears them, hears the
 * television, and hears half of what you say to someone else — and because
 * every fragment it catches becomes a turn, the transcript fills with
 * questions nobody asked while you are trying to work.
 *
 * With this on, nothing is transcribed unless SPACE is held. The wake word
 * still works while the key is down, so "hey jarvis" costs nothing, and
 * everything else about the loop is unchanged.
 *
 * Set VITE_PUSH_TO_TALK=false to go back to always listening.
 */
export const PUSH_TO_TALK = flag(
  'VITE_PUSH_TO_TALK',
  import.meta.env.VITE_PUSH_TO_TALK,
  true,
)

/**
 * How long the microphone stays open after the key comes up.
 *
 * Not cosmetic. Speech is segmented by the voice detector and transcribed
 * afterwards, and `transcribe` re-checks the mode when the audio comes back —
 * so going deaf the instant the key is released throws away the sentence that
 * was just spoken into it. This window covers the tail of the segment plus the
 * round trip, and it is why releasing on the last syllable still works.
 */
export const PTT_RELEASE_GRACE_MS = 2500
